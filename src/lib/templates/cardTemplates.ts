...
const romanceTemplate: CardTemplate = {
  ...
  build: () => {
    ...
    if (schemaTemplate) {
      (base.data.extensions as Record<string, unknown>).mvuzod = { schema: schemaTemplate.schema };
    }
...
export function applyCardTemplate(template: CardTemplate, existingCard: CharacterCardV3): CharacterCardV3 {
  ...
  if (built.data) {
    ...
    if (built.data.extensions) {
      const ext = built.data.extensions as Record<string, unknown>;
      const targetExt = clone.data.extensions as Record<string, unknown>;
